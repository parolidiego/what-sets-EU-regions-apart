...
eu_data <- eu_data |> 
  mutate(continental_region = as.factor(case_when(
    country_code %in% c("AUT", "BEL", "DEU", "FRA", "LUX", "NLD", "IRL") ~ "Western",
    country_code %in% c("GRC", "ESP", "ITA", "MLT", "PRT") ~ "Southern",
    country_code %in% c("DNK", "EST", "FIN", "LTU", "LVA", "SWE") ~ "Northern",
...
plot_boxplot(eu_data, by="continental_region", ncol = 2, nrow = 5)
```
...
Third: set the reference class.

```{r}
# Set the reference class as the one with the most observation
...
As we can see we are assigning each observation to the most common class and we already get a 58% accuracy.
...
```
```{r}
head(fitted(multinomial_classification))
```
```{r, echo=FALSE}
options(scipen = 0)
```
...
Talk about accuracy etc.

Which were the most important variables in the model? Explain what is being done below
...
We already have a really good model so accuracy seems like a good metric to maximize

Computational times do not allow to put very large grids for machine learning methods, however small refinements of the grid have been carried out thorughout all the methods below. 
...
partial(knn_classification, pred.var = "population_exposure_to_pm2_5",
        which.class="Western",
...
Two clear pattern for `employment_to_population` seems like for the initial range is flat then around 65% it either decreases sharply (for Eastern and Southern) or increases sharply (for Western and Northern). This indicates that Southern and Eastern Europe tend to have low `employment_to_population` (below 65%), while Western and Northern tend to have high `employment_to_population` (above 65%).
...
No feature selecting for machine learning methods as it is not needed, they learn by themselves the p
...
predictions_linear <- predict(linear_regression,
                              newdata=testing_scaled)
round(postResample(pred = predictions_linear,  obs = testing_scaled$gross_domestic_product),3)
```
...
Able to reduce AIC and adj R squared. Comment on variable chosen

Do it with caret now but maximizing RMSE
...
It seems like the model is actually not performing any kind of feature selection. It is keeping all the variables.
...
Computational times do not allow to put very large grids for machine learning methods, however small refinements of the grid have been carried out throughout all the methods below. 

```{r}
param_grid = expand.grid(kmax = c(3, 5, 7, 9), # Number of neighbors considered
...
Seems to have a quasi linear relationship and then it platous a bit 
...
Again confirms labour prodctivity as a very important variable
...
predictions_rf <- predict(rf_regression,
                           newdata=testing,
                           type="raw")
round(postResample(pred = predictions_rf,  obs = testing$gross_domestic_product),3)
```
...
`labour_productivity` has a very similar pattern as the one that was found for KNN.
...
predictions_nn <- predict(nn_regression,
                           newdata=testing,
                           type="raw")
round(postResample(pred = predictions_nn,  obs = testing$gross_domestic_product),3)
```

```{r}
nn_imp <- varImp(nn_regression)
plot(nn_imp, top=10)

partial(nn_regression, pred.var = "labour_productivity", 
        plot = TRUE, rug = TRUE)
```
